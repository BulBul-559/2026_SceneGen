...
def run_build_vision_dataset(
    ...
    *,
    ...
    for index, scene_dir in enumerate(candidates, start=1):
        missing = missing_required_files(scene_dir, require_maps=require_maps)
        if missing:
            skipped += 1
            records.append({"scene_key": scene_dir.name, "status": "skipped", "reason": "missing_required_files", "missing": missing})
            continue
        try:
            record = build_scene_dataset_entry(scene_dir, dataset_dir, run_dir, overwrite=overwrite)
        ...
        if log_every and index % log_every == 0:
            log_fn(f"processed={index}/{len(candidates)} copied={copied} skipped={skipped} failed={failed}")
...
def main() -> int:
    args = parse_args()
    report = run_build_vision_dataset(
        args.run_dir,
        args.dataset_dir,
