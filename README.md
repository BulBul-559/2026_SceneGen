...
- 输出批量 `label/*.json` v1.1 和 `label/report/*_report.json`，记录 BS/UE 点位、分组、快速 positions 数组和验证结果；根目录 `label.json` 保留为首个 label 的兼容入口。
...
- 每个场景同步生成 floorplan：
  ...
  - 首个 label 的兼容叠加图 `floorplan/label_overlay.png`
...
- `label/report/*_report.json`: 每份 label 的生成与验证报告，记录每个 room 的点位数量、跳过原因、错误和警告。
- `label.json` / `label_report.json`: 兼容入口，内容等于本次批量生成的第一份 label。
...
- `floorplan/label_overlay.png`: 兼容入口，内容等于第一份 label 的检查图。
...
- `label/report/<name>_report.json`: 记录每个 room 的 floor source、UE/BS 数量、验证结果和跳过原因。
- `label_floorplan/<name>.png`: 将对应 label 的 UE/BS 绘制到 `geometry_raw.png` 上，便于人工检查。
- `label.json`、`label_report.json`、`floorplan/label_overlay.png`: 首个 label 的兼容副本。
