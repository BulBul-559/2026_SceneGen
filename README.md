...
4. 从 `data/3D-Front/scenegen_manifest.json` 的 3D-FUTURE 物体池中按 `procedural.room_profiles` 配置的房间家具类别序列和 semantic filter 筛选家具；默认 room type 覆盖客厅、卧室、餐厅、书房、厨房、卫浴和走廊/玄关，并通过 `procedural.asset_reuse` 控制同一模型在 room/scene 内的重复使用；每个 run 会在 `procedural_report.json` 中写入 asset pool coverage，说明各 room type/class 过滤后有多少候选资产。
