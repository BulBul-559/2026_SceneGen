...
运行时默认读取 `config/template.yaml`。CLI 覆盖统一使用可重复的 `--set key.path=value`，value 会按 YAML 解析。每次运行会在结果目录写出最终生效配置：
...
配置合并顺序是：代码内置默认值、`--config` 指定 YAML、CLI `--set` 覆盖、路径与类型归一化、字段和值校验。配置 v2 不兼容旧 YAML 字段和旧显式 CLI 参数；未知字段会直接报错。`config/` 目录只保留 `template.yaml`，需要实验配置时建议复制到其他位置或通过 `--set` 覆盖。
...
```bash
uv run scenegen --set pipeline.mode=front3d --set pipeline.scenes=1 --set pipeline.run_name=smoke_front3d
```
