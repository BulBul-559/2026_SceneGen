...
- 支持实验性 `procedural_front3d` 模式，自动生成类 3D-FRONT 多房间建筑并摆放 3D-FUTURE 家具资产。
- 支持 `procedural_front3d_vision` 模式，跳过大体积 OBJ/XML/assets 输出，面向 VisionEncoder 预训练直接生产 floorplan、label 和 maps。
...
SceneGen/
  config/
    ...
    procedural_front3d.yaml # 自动场景生成实验模板
    procedural_front3d_vision.yaml # 程序化视觉数据专用模板
  data/
  ...
  docs/
    ...
    procedural_worker_sweep_20260617.md # 自动随机生成 worker 扫参记录
  src/scenegen/
...
- [config/procedural_front3d.yaml](/home/sunmeiyuan/projects/SceneGen/config/procedural_front3d.yaml)
- [config/procedural_front3d_vision.yaml](/home/sunmeiyuan/projects/SceneGen/config/procedural_front3d_vision.yaml)

配置字段、可选值和 CLI 覆盖说明：

[config/README.md](/home/sunmeiyuan/projects/SceneGen/config/README.md)

运行时默认读取 `config/bistro.yaml`。3D-FRONT 合成建议用 `--config config/front3d.yaml`；自动生成类 3D-FRONT 场景使用 `--config config/procedural_front3d.yaml`；只需要平面图、label 和 maps 的随机生成数据使用 `--config config/procedural_front3d_vision.yaml`。这些 YAML 是按工作流拆分的一级基础模板，只保留当前任务相关字段；未写字段由 `DEFAULT_CONFIG` 补齐，并会写入最终的 `effective_config.yaml`。CLI 覆盖统一使用可重复的 `--set key.path=value`，value 会按 YAML 解析。每次运行会在结果目录写出最终生效配置：
...
```bash
uv run scenegen-batch --config config/tasks/procedural_front3d_vision_full_simulation.yaml --set pipeline.scenes=60 --set pipeline.run_name=procedural_front3d_vision_test_60
```
...
该模板定位为随机生成数据的任务级配置，只保留 3D-FUTURE 家具池来源、`procedural` 户型/摆放规则、label、floorplan、maps 和 batch 等生成链路相关字段；Bistro 字段和复现已有 Front3D 场景的 scene selection/precheck 字段不会出现在这个模板中。它默认使用 `batch.workers: 24`、`batch.task_timeout_s: 600`，默认开启 `maps.enabled`，BS 数量策略和 Front3D 全量模板一致，使用 `label.bs.count.strategy: area_adaptive` 按房间面积自适应生成。

只需要视觉监督数据时使用 `config/tasks/procedural_front3d_vision_full_simulation.yaml`。它复用程序化户型、家具摆放、precheck、quality、label、floorplan 和 maps，并在模板中显式固定当前生产用 `procedural` 户型与摆放参数；`output.profile: vision_only` 会强制关闭 `scene.obj`、`scene.xml`、`assets/` 和 OBJ summary；默认 `batch.workers: 48`；每个 scene 只生成 `label_panel_0p5`，maps 默认也以它作为 BS 来源；仍保留小体积的 `procedural_source/` 与 `placements.json` 作为可追溯源记录。该模板默认使用 `procedural.room_count_ranges` 按 15% / 70% / 15% 采样 1-2、3-7、10-20 间房，并将 `required_room_types` 置空以允许小房间样本。

三个 full 生产模板默认会在每个 scene 的标准生成流程内同步生成 `geometry.npz` 和 BS-UE `pair_cache.npz`；batch 收尾阶段会按顶层 `maps` 配置做补漏/跳过检查。如果还要整理 compact vision dataset，只需额外开启 dataset：
...
`maps` 是标准 scene 生成阶段，普通 `scenegen` 单场景入口和 `scenegen-batch` worker 都会执行；基础配置默认关闭，full 生产任务模板会覆盖为 `maps.enabled: true`。`postprocess` 现在只保留 batch 收尾的 dataset 构建和 maps 补漏/跳过检查。

## 自动场景生成

`procedural_front3d` 是自动场景生成的第一版 baseline，目标是提供接近无限的类 Front3D 样本。它当前按阶段执行：

1. 按 `procedural.layout` 采样多房间户型：默认 `mixed` 会按 `procedural.layout_weights` 在多个 layout 中加权采样；`split_tree` 会从完整 apartment footprint 递归切分房间，`grid` 可用于规整行列对照，`rect_union` 会由多个相邻房间矩形拼出 L/T/凹口式不规则外轮廓，`room_graph` 会先生成树状房间邻接关系再把新房间挂到已有房间边上，`polygon_shell` 会先采样带凹口的外部 shell 再在 shell 内切分房间，`corridor_spine` 会生成走廊 spine + 两侧房间的公寓式拓扑；room 类型由 `procedural.room_types`、`procedural.required_room_types`、`procedural.room_type_weights`、`procedural.room_type_max_counts` 和 `procedural.room_type_assignment` 控制，默认保证至少有客厅、卧室和厨房，并限制厨房、餐厅、卫浴等类型的最大出现次数，再通过 `geometry_fit` 复用 room type 几何预检规则匹配更合适的房间；随后生成 floor、ceiling、wall、door 和外墙 window mesh。外墙按 room union 的真实边界生成，因此 `rect_union` / `room_graph` / `polygon_shell` 的凹口不会被整体 bbox 外墙补回矩形。
...
4. 从 `data/3D-Front/scenegen_manifest.json` 的 3D-FUTURE 物体池中按 `procedural.room_profiles` 配置的房间家具类别序列、`required_classes` 和 semantic filter 筛选家具；默认 room type 覆盖客厅、卧室、餐厅、书房、厨房、卫浴和走廊/玄关，并通过 `procedural.asset_reuse` 控制同一模型在 room/scene 内的重复使用；每个 run 会在 `procedural_report.json.asset_pool_coverage` 和独立的 `procedural_asset_pool_coverage.json` 中写入 asset pool coverage，说明各 room type/class 过滤后有多少候选资产。
5. 先按 `procedural.placement_groups` 尝试生成局部关系组，例如客厅茶几/沙发组合、餐厅餐桌椅组合、卧室床 + 床头柜组合、书房书桌椅组合、厨房台面/椅凳组合；失败时不会丢弃目标 class，会回落到普通逐个摆放。
6. 按 `procedural.placement_policy` 采样剩余家具位置：默认床/柜类 `floor` 物体更靠墙，`table` 更靠房间中心，`seat` 保持自由采样；`placement_policy.by_room_type` 会进一步按房型覆盖，例如厨房/卫浴柜体靠墙、客厅/餐厅桌椅更靠中心。
7. 在每个 room 内进行 bbox 约束摆放，避免越界、家具间碰撞和门洞通行区域被家具堵住。
8. 按 `procedural.precheck` 检查实际家具数、全局和单房间目标完成率、跳过比例、房间连通性、户型 footprint 质量、room topology 复杂度、基础房间几何质量和 room type 几何规则；失败时自动换 seed 重试补齐同一输出编号。
9. 复用现有 `scene.obj`、`scene.xml`、`label/`、`floorplan/`、`class_mask`、quality 和 statistics 输出链路。

第一版仍是规则 baseline：房间拓扑和关系约束还比较轻量；建筑结构已支持外墙窗户玻璃面，并会输出相邻房间的连通关系和门洞位置；room 类型支持 `procedural.required_room_types` 必选组成、`procedural.room_type_weights` 加权分布、`procedural.room_type_max_counts` 类型上限和 `procedural.room_type_assignment: geometry_fit` 几何规则匹配；家具数量已经支持 `procedural.object_count` 面积自适应和 `object_count.by_room_type` 房型密度覆盖，家具组合已经迁移到 `procedural.room_profiles`，可以按房间类型配置 `table`、`seat`、`floor` 的组合序列、核心 `required_classes`，并可用 `category`、`super_category`、`name`、`material` 关键词筛选更合适的资产；默认客厅 table 会优先筛选 coffee/tea/side table，配合 sofa 类 seat 形成会客区；`procedural.asset_reuse` 会减少同一 3D-FUTURE model 在 room/scene 内重复出现；位置采样支持 `procedural.placement_policy` 的中心/靠墙偏置和 `placement_policy.by_room_type` 房型覆盖，局部组合支持 `procedural.placement_groups` 的 anchor + companion 关系；`procedural.door_clearance_m` 会保护门洞周围通行区域。程序化预检会在 label/floorplan 前过滤整体、单房间或单家具类别摆放过少、失败比例过高、room 必需类别缺失、room adjacency 不连通、footprint fill ratio 过低或过高、凹口面积超出配置、room topology 边数/叶子房间/分支房间/图直径不符合配置、room 过小、长宽比过大、room type 几何尺寸不合理或资产复用过多的样本。run 根目录的 `procedural_report.json` 会汇总实际 layout/配置 layout 计数、room 数量、room type、面积/长宽比、footprint fill ratio/凹口面积、room topology 度数/连通分量/叶子房间/图直径、adjacency/window、家具完成率、desired/placed/skipped 资产类别分布、按 room type 统计的实际摆放类别、asset pool coverage、唯一模型比例、重复模型数量、top reused models、跳过原因、关系组成功次数、precheck 通过率、room type 几何问题统计，以及被 precheck 跳过的 attempts 按 layout 和 error code 的汇总；其中 asset pool coverage 也会单独写为 `procedural_asset_pool_coverage.json`，便于批量生产后脚本化质检。后续可以逐步替换为更强的 room planner、语义资产组、约束求解器和可达性验证。
...
大规模生产使用 `scenegen-batch`，它会先固化 `scene_plan.jsonl`，再按 worker 分片执行，支持 resume、失败队列、重试队列、worker 日志和统一 batch manifest。目前 batch 支持 `front3d`、`procedural_front3d` 和 `procedural_front3d_vision`：
...
开启 `maps.enabled` 后，每个成功 scene 会额外得到 `maps/geometry.npz`、`maps/pair_cache.npz` 和 `maps/metadata.json`。`pair_cache.npz` 按 scene 缓存 bucket-balanced BS-UE pair 标签，包含像素/米制坐标、LoS、截断/原始穿墙段数、距离和 BS snap 信息；旧版 dense `maps/propagation.npz` 默认不再写出，可用 `maps.write_propagation=true` 显式保留。开启 `postprocess.dataset.enabled` 后，默认会在 `datasets/<run_name>_vision/` 下构建 compact vision dataset，只保留训练需要的 floorplan、mask、geometry、pair cache、BS label 和 metadata。

`manifest_batch.json`、`manifest.json` 和 `manifest_<mode>.json` 会在 batch 完成后统一汇总最终发布到 run 根目录的标准场景目录，例如 `front3d_0000/`、`procedural_front3d_0000/` 或 `procedural_front3d_vision_0000/`。

`scenegen-batch` 默认读取 YAML 的 `batch.workers`、`batch.scheduler` 和 `batch.max_retries`；命令行 `--workers`、`--scheduler`、`--max-retries` 仍可临时覆盖。Front3D 和 procedural full 任务模板默认使用 24 worker；procedural vision full 模板默认使用 48 worker；三者都使用 hybrid 调度并开启 maps。`hybrid` 会先按固定分片运行，只有当某个 worker 自己队列清空且其他队列仍有待处理任务时，才从剩余任务最多的队列偷取尾部任务；`static` 严格保持固定分片；`dynamic` 使用共享任务队列。batch 子进程会跳过自己的 `summary/` 汇总复制，最终只由 batch 顶层统一生成 summary；成功 scene 会从 `batch/worker_runs` 直接 move 到 run 根目录，`batch/worker_runs` 主要保留 worker 子 run 的日志、配置和失败场景调试信息，不再保存成功场景的完整重复副本。
...
results/<run_name>/
  ...
  statistics.json
  procedural_report.json        # procedural_front3d / procedural_front3d_vision 模式
  procedural_asset_pool_coverage.json  # procedural_front3d / procedural_front3d_vision 模式
  visual_index.html             # run 级可视化索引页
  manifest.json
  manifest_bistro.json、manifest_generated.json、manifest_front3d.json、manifest_procedural_front3d.json 或 manifest_procedural_front3d_vision.json
  summary/
    obj/
      copy_manifest.json
      bistro_0000.obj
...
- 新增配置项时，应同步更新：
  - `config/bistro.yaml`、`config/front3d.yaml`、`config/procedural_front3d.yaml`、`config/procedural_front3d_vision.yaml` 这些按工作流聚焦的基础模板
  - `config/tasks/*.yaml` 中和该任务真实相关的执行字段
  - `config/README.md`
  - `README.md`
