...
1. 按 `procedural.layout` 采样多房间户型：默认 `split_tree` 会从完整 apartment footprint 递归切分房间，`grid` 可用于规整行列对照；room 类型由 `procedural.room_types`、`procedural.required_room_types` 和 `procedural.room_type_weights` 控制，默认保证至少有客厅、卧室和厨房，并让客厅和卧室更常见；随后生成 floor、ceiling、wall、door 和外墙 window mesh。
