...
SceneGen/
  config/
    bistro.yaml           # Bistro 默认完整模板，也是默认 YAML 入口
    front3d.yaml          # 3D-FRONT 完整模板
  data/
...
运行时默认读取 `config/bistro.yaml`。3D-FRONT 合成建议用 `--config config/front3d.yaml`。CLI 覆盖统一使用可重复的 `--set key.path=value`，value 会按 YAML 解析。每次运行会在结果目录写出最终生效配置：
...
配置合并顺序是：代码内置默认值、`--config` 指定 YAML、CLI `--set` 覆盖、路径与类型归一化、字段和值校验。配置 v2 不兼容旧 YAML 字段和旧显式 CLI 参数；未知字段会直接报错。需要实验配置时建议复制 `config/bistro.yaml` 或 `config/front3d.yaml` 到其他位置，再通过 YAML 或 `--set` 覆盖。
...
results/<run_name>/
  ...
  bistro_0000/
    ...
    floorplan/
      ...
      stack.npz
      meta.json
      semantic.png        # 仅 floorplan.semantic.enabled: true 时生成
      semantic.json       # 仅 floorplan.semantic.enabled: true 时生成
...
3D-FRONT 的 UE 默认使用 `label.ue.sampling_domain: global_floor` 与 `label.ue.variants.connected: [true]`：`panel` 和 `walk` 都会先在建筑 XY bbox 的全局矩形网格上采样，再扣除 outdoor 和按 `label.ue.wall_clearance_m` 膨胀后的 wall，最后才把点按 room floor mesh 归属到各个 room；归属不到任何 room 但仍在 free space 上的点会进入 `label.ue.connected_area.room_id: "__corridor__"` 的 connected area group。默认 `label.ue.wall_clearance_m: 0.2`，这个采样会按 `front3d.openings.mode` 把 3D-FRONT 原始门洞在墙体膨胀前标为 free space，但不会在墙体膨胀后额外恢复门洞；门洞足够宽就自然保留采样点，否则会被膨胀后的 wall 吃掉。旧的逐 room 采样可切回 `label.ue.sampling_domain: room_floor`，或设置 `label.ue.variants.connected: [false]` 只保留 room 内点。

`panel` 表示室内平面采样：只扣除 outdoor 和墙体间隔，不扣家具。

`walk` 表示可行走区域：它在 `panel` 的基础上继续扣除家具障碍，默认使用 `label.ue.obstacle_strategy: height_aware` 做高度感知过滤；如需保守的整列占用行为，可切换为 `footprint_column`。同时会忽略高度低于 `label.ue.ignore_low_obstacles_below_m` 的薄物体，并删除小于 `label.ue.min_component_area_m2` 的孤立小区域。默认参与 walk 扣除的 placement class 是 `table`、`seat` 和 `floor`。

label 支持批量生成：`label.ue.variants.strategies`、`label.ue.variants.grid_m` 和 `label.ue.variants.connected` 会做笛卡尔组合。例如：
...
上面会生成 4 份 label：`label_panel_connected_0p1`、`label_panel_room_0p1`、`label_walk_connected_0p1`、`label_walk_room_0p1`。其中 `connected` 表示保留 connected area group，`room` 表示只保留 room 内点。只有一个 connected area 模式时，文件名为 `label_panel_0p1` / `label_walk_0p1`。

BS 默认每个有效 room 最多 4 个，优先放在墙边或角落附近；也可以把 `label.bs.count.strategy` 切换为 `area_adaptive`，按房间 floor 面积决定每个 room 的 BS 数量，小房间可不放，大房间可放更多。若要评测单基站定位性能，可以改用几何中心 BS：
...
当前 floorplan 有两种输出。默认生成几何占据图，语义平面图默认关闭，需要时可用 `floorplan.semantic.enabled: true` 打开。
...
默认几何平面图使用高密度单高度方案：`floorplan.resolution_m: 0.05`、`floorplan.sampling.density_scale: 128.0`、`floorplan.height.values_m: [1.6]`。这类输出偏几何占据图，不包含资产类别语义。由于原始投影来自随机表面采样，低密度时可能有点状采样噪声；当前默认通过提高采样密度减轻这类伪纹理。
