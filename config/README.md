...
| `layout_weights` | mapping | `{room_graph: 0.25, corridor_spine: 0.25, rect_union: 0.20, polygon_shell: 0.15, split_tree: 0.10, grid: 0.05}` | 仅在 `layout: mixed` 时生效。权重必须非负，至少一个支持的 layout 权重大于 0；每个 scene 的实际 layout 会写入 manifest 和 `procedural_report.json`。YAML 是深合并，未写出的 layout weight 会继承模板/默认值。 |
...
| `min_class_placement_ratio` | mapping / `null` | `{}` | 按 furniture class 设置最低完成率，例如 `{seat: 0.6, table: 0.5}`；只检查本次目标中实际出现的 class。空映射或 `null` 表示不启用。 |
...
topology、class placement 和 asset diversity 阈值默认多为空值，不会改变模板分布。需要做数据集分桶或强约束时，可以按 `procedural_report.json` 中观察到的 `topology`、`placed_class_counts_total`、`unique_model_ratio`、`duplicate_model_count` 分布再反向设置这些阈值。
...
每次 `procedural_front3d` run 会在 `procedural_report.json.asset_pool_coverage` 和独立的 `procedural_asset_pool_coverage.json` 中记录每个 room type + furniture class 的 `pool_count`、`candidate_count`、`filter_matched` 和 `fallback_to_unfiltered`。`asset_pool_empty_candidate_count` 用于发现某类资产池完全为空；`asset_pool_fallback_filter_count` 用于发现某些 semantic filter 太窄、实际回退到了未过滤资产池。
