...
| 字段 | 可选值 / 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `layout` | `grid` / `split_tree` / `rect_union` | `split_tree` | 户型布局策略。`grid` 是早期规整行列；`split_tree` 从完整 apartment footprint 递归切分房间；`rect_union` 从连通房间矩形集合拼出 L/T/凹口式不规则外轮廓，适合增加户型边界多样性。 |
