...
- `procedural_front3d`: 实验性自动生成模式。按 `procedural.layout` 采样多房间户型，默认 `split_tree` 从完整 apartment footprint 递归切分房间；随后写出 Front3D-like 建筑 `procedural_source/scene.json` / `architecture.obj`，再按 `procedural.object_count` 计算每房家具数量，并从 3D-FUTURE 资产池中按 `procedural.room_profiles` 的类别序列和 semantic filter 摆放家具。位置采样由 `procedural.placement_policy` 控制，默认 `floor` 靠墙、`table` 靠中心、`seat` 自由采样；餐厅/书房等可通过 `procedural.placement_groups` 先生成 anchor + companion 关系组，例如餐桌椅或书桌椅。生成后会按 `procedural.precheck` 检查摆放数量完成率，失败时换 seed 重试同一编号；通过后复用 front3d 的 OBJ/XML、label、floorplan、class mask、quality 和 statistics 输出链路。
