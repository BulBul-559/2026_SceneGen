...
- `procedural_front3d`: 实验性自动生成模式。按 `procedural.layout` 采样多房间户型，默认 `split_tree` 从完整 apartment footprint 递归切分房间；默认 room type 覆盖客厅、卧室、餐厅、书房、厨房、卫浴和走廊/玄关，并可通过 `procedural.room_type_weights` 控制分布权重。随后写出 Front3D-like 建筑 `procedural_source/scene.json` / `architecture.obj`，包括 floor、ceiling、wall、door、外墙 window mesh、room adjacency 和门洞 bbox；再按 `procedural.object_count` 计算每房家具数量，并从 3D-FUTURE 资产池中按 `procedural.room_profiles` 的类别序列和 semantic filter 摆放家具。位置采样由 `procedural.placement_policy` 控制，默认 `floor` 靠墙、`table` 靠中心、`seat` 自由采样；餐厅、卧室、书房等可通过 `procedural.placement_groups` 先生成 anchor + companion 关系组，例如餐桌椅、床 + 床头柜或书桌椅。生成后会按 `procedural.precheck` 检查摆放数量完成率、room adjacency 连通性、最小 room 面积和最大长宽比，失败时换 seed 重试同一编号；通过后复用 front3d 的 OBJ/XML、label、floorplan、class mask、quality 和 statistics 输出链路。
