...
- `procedural_front3d`: 实验性自动生成模式。按 `procedural.layout` 采样多房间户型，默认 `split_tree` 从完整 apartment footprint 递归切分房间；随后写出 Front3D-like 建筑 `procedural_source/scene.json` / `architecture.obj`，再从 3D-FUTURE 资产池中按 `procedural.room_profiles` 的类别序列和 semantic filter 摆放家具，复用 front3d 的 OBJ/XML、label、floorplan、class mask、quality 和 statistics 输出链路。
